...
export class OpenSeaClient {
    ...
    try {
      ...
      if (err?.name === 'AbortError') {
        throw new OpenSeaError(
          `OpenSea request timed out after ${timeoutMs / 1000}s: ${url}`,
          'NETWORK_TIMEOUT'
        );
      }
      throw err;
  ...
  async fetchFreeKey() {
    ...
    try {
      ...
      throw new OpenSeaError(
        `Could not reach OpenSea to create an instant API key [${code}: ${detail}]. ` +
          (hint ? `${hint} ` : '') +
    ...
    if (!res.ok) {
      if (res.status === 429) {
        throw new OpenSeaError(
          'Could not create an instant OpenSea key: rate limited (HTTP 429). ' +
            'OpenSea rate-limits key creation per IP. Set OPENSEA_API_KEY manually from ' +
            'opensea.io -> Settings -> Developer.',
          'KEY_RATE_LIMITED'
        );
      }
      throw new OpenSeaError(
        `Could not create an instant OpenSea key (HTTP ${res.status}). ` +
  ...
  async ensureApiKey() {
    ...
    console.warn(
      '[opensea] No OPENSEA_API_KEY set; creating a temporary instant key. ' +
        'OpenSea free-tier instant keys expire after 30 days and key creation is rate-limited per IP. ' +
        'For production, set OPENSEA_API_KEY.'
    );
      ...
      if (res.status === 401 || res.status === 403) {
        if (!this.suppliedKey && attempt < MAX_RETRIES) {
          ...
          try {
            ...
            // Fall through to the actionable key error below.
          }
        }
        if (optional) return null;
        throw new OpenSeaError(
          `OpenSea rejected the API key (HTTP ${res.status}) on ${path}. ` +
            `If it is an expiring key, replace it and update OPENSEA_API_KEY.`,
          'KEY_REJECTED'
        );
      }
