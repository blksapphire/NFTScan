...
export class Telegram {
  ...
  /**
   * Fetch messages sent to the bot since `offset`.
   * timeout: 0 keeps this a short poll — long-polling inside a cron run would
   * just burn CI minutes waiting.
   */
  async getUpdates(offset) {
    ...
    try {
      ...
      // Never let a command-polling failure stop the alerting path.
...
/** ▰▰▰▰▰▰▰▱▱▱ — a score you can read at a glance on a phone. */
...
/**
 * The alert card. Every score is shown with its reasoning, so you can calibrate
 * the filters against what actually happens rather than trusting a number.
 */
export function formatAlert(candidate, scored, nowMs = Date.now()) {
  const lines = [];

  lines.push(`<b>${escapeHtml(headerFor(candidate, nowMs))}</b>`);
  lines.push('');
  lines.push(`<b>${escapeHtml(candidate.name)}</b>`);
  ...
  if (candidate.mintPriceEth !== null && candidate.mintPriceEth !== undefined) {
    const price = formatEth(candidate.mintPriceEth);
    details.push(
      `💰 Mint: <b>${escapeHtml(price)}</b>${candidate.isNativeCurrency === false ? ' (paid in a token, not ETH)' : ''}`
    );
  }
  if (Number.isFinite(candidate.maxPerWallet) && candidate.maxPerWallet > 0) {
    details.push(`🎫 Max ${candidate.maxPerWallet} per wallet`);
  }
  if (Number.isFinite(candidate.startTimeMs) && candidate.kind === 'upcoming') {
    details.push(`🕐 Opens ${escapeHtml(new Date(candidate.startTimeMs).toUTCString())}`);
  }
  if (Number.isFinite(candidate.mintsPerMinute)) {
    details.push(
      `📈 ${candidate.mintsPerMinute.toFixed(1)} mints/min · ${candidate.uniqueMinters} wallets`
    );
  }
  if (details.length) {
  ...
  // Risk findings are separated out and put first. They are the reasons to walk
  // away, so burying them as bullet six of eight would defeat the point —
  // especially for a candidate that cleared the threshold in spite of one.
  ...
  if (risks.length) {
    ...
    for (const risk of risks) {
      lines.push(`• <b>${escapeHtml(risk.replace(/^RISK:\s*/i, ''))}</b>`);
    }
    lines.push('');
  }
  ...
  // Which components were unavailable matters: it tells you how much to trust
  // the number. A score from three signals is weaker than one from six.
  if (scored.available?.length) {
    const coverage = `Scored on ${scored.available.length}/6 signals`;
    const thin = scored.available.length <= 4;
  ...
  if (candidate.socials?.twitter) {
    links.push(`<a href="https://x.com/${encodeURIComponent(candidate.socials.twitter)}">X</a>`);
  }
  if (candidate.socials?.discord) {
    links.push(`<a href="${encodeURI(candidate.socials.discord)}">Discord</a>`);
  }
  if (candidate.socials?.website) {
    links.push(`<a href="${encodeURI(candidate.socials.website)}">Website</a>`);
  }
  if (links.length) lines.push(links.join(' · '));

  if (candidate.contractAddress) {
    lines.push(`<code>${escapeHtml(candidate.contractAddress)}</code>`);
  }

  lines.push('');
  lines.push(
    '<i>Not financial advice. Verify the contract yourself before spending — this bot checks metrics, not honesty.</i>'
  );
...
/**
 * Read and act on commands sent since the last run.
 * Mutates `state.overrides` and `state.telegramOffset`.
 * @returns {Promise<string[]>} descriptions of what changed, for logging
 */
export async function processCommands(tg, state, cfg) {
  ...
  for (const update of updates) {
    // Advance the cursor even for messages we ignore, or they replay forever.
    ...
    // Only obey the configured owner. Anyone who finds the bot could message it.
    ...
    switch (command) {
      case '/start':
      case '/help':
        await tg.send(HELP, { chatId: replyTo });
        break;
      ...
      case '/status': {
        ...
        await tg.send(
          [
            '<b>Status</b>',
            '',
            `Alerting: <b>${state.overrides?.paused ? 'PAUSED' : 'active'}</b>`,
            `Score threshold: <b>${state.overrides?.minScore ?? cfg.minScore}</b>`,
            `Chain: ${escapeHtml(cfg.chain || 'ethereum')}`,
            `Mode: ${escapeHtml(cfg.mode)}`,
            '',
            `Runs: ${s.runs ?? 0}`,
            `Alerts sent: ${s.alertsSent ?? 0}`,
            `Last run: ${s.lastRunAt ? escapeHtml(s.lastRunAt) : 'never'}`,
            `Last alert: ${s.lastAlertAt ? escapeHtml(s.lastAlertAt) : 'never'}`,
            `Contracts remembered: ${Object.keys(state.alerted || {}).length}`,
          ].join('\n'),
          { chatId: replyTo }
        );
        break;
      ...
      case '/threshold': {
        ...
        if (!Number.isFinite(value) || value < 0 || value > 100) {
          await tg.send('Usage: /threshold 75  (a number from 0 to 100)', { chatId: replyTo });
          break;
        }
      ...
      case '/pause':
        state.overrides = { ...state.overrides, paused: true };
        applied.push('paused');
        await tg.send('Paused. Still watching, not alerting. /resume to turn back on.', {
          chatId: replyTo,
        });
        break;

      case '/resume':
        state.overrides = { ...state.overrides, paused: false };
        applied.push('resumed');
        await tg.send('Alerting again.', { chatId: replyTo });
        break;

      case '/recent': {
        const recent = Array.isArray(state.recent) ? state.recent.slice(0, 10) : [];
        if (recent.length === 0) {
          await tg.send('No alerts yet.', { chatId: replyTo });
          break;
        }
        await tg.send(
          ['<b>Recent alerts</b>', '']
            .concat(
              recent.map(
                (r) =>
                  `${r.score}/100 — ${escapeHtml(r.name)} <i>(${escapeHtml(r.kind)})</i>` +
                  (r.contractAddress ? `\n<code>${escapeHtml(shortAddress(r.contractAddress))}</code>` : '')
              )
            )
            .join('\n'),
          { chatId: replyTo }
      ...
      default:
        await tg.send(`Unknown command ${escapeHtml(command)}. Try /help`, { chatId: replyTo });
    }
  }
